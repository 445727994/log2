...
//已付款：指订单已付款，但还未确认收货 已收货：指订单已确认收货，但商家佣金未支付 已结算：指订单已确认收货，且商家佣金已支付成功 已失效：指订单关闭/订单佣金小于0.01元，订单关闭主要有：1）买家超时未付款； 2）买家付款前，买家/卖家取消了订单；3）订单付款后发起售中退款成功；3：订单结算，12：订单付款， 13：订单失效，14：订单成功
    ...
    public function orderCommit($result,$useField=['ordernum','status','is_complete','relation_id','commission_amount','alimama_share_fee']){
        ...
        if(!$orderMsg){
            ...
            TbkOrder::query()->insert($orderMsg);
            $orderMsg=TbkOrder::query()->where(['ordernum'=>$result['trade_id']])->select($useField)->first();
        }
    ...
    public function calcuate($orderMsg,$server_percent='0.1'){
        $time=date('Y-m-d H:i:s');
        //统一加收技术服务费
        $orderMsg->tb_cost= $orderMsg->tb_cost==0?$orderMsg->commission_amount*$server_percent:$orderMsg->tb_cost;
        $orderMsg->user_cost=$orderMsg->tb_cost?:0;

        //计算金额
        if($orderMsg->alimama_share_fee<$orderMsg->commission_amount*$server_percent){
            $orderMsg->platform=$orderMsg->commission_amount*$server_percent-$orderMsg->alimama_share_fee;
            $orderMsg->calcuate_money=$orderMsg->commission_amount-$orderMsg->commission_amount*$server_percent;
        }else{
            $orderMsg->platform=0;
            $orderMsg->calcuate_money=$orderMsg->commission_amount-$orderMsg->alimama_share_fee;
        }

        //统计扣除平台费用  平台暂时不做扣除
        //$orderMsg->platform=floor($orderMsg->calcuate_money*0.05);
        //get_type  1自己2下级3团队
        ...
        $orderMsg->user_id=$user->id;
        $countPercent=0;
        $LevelMsg=Level::LevelMsg($user->level_id);
        $credit=$orderMsg->calcuate_money*$LevelMsg['commission']*0.01;
        $userLogs[]=['ordernum'=>$orderMsg->ordernum,'user_id'=>$user->id,'type'=>1,'credit'=>$credit,
            'get_type'=>1,'status'=>1,'remark'=>'订单返利：'.$credit,'created_at'=>$time,'updated_at'=>$time];
        $countPercent+=$LevelMsg['commission'];
        //上级反
        if($user->level_id && $user->invited_id){
            $upUser=User::getUserById($user->invited_id);
            $upLevel=Level::LevelMsg($upUser->level_id);
            $credit=$orderMsg->calcuate_money*$upLevel['up']*0.01;
            $userLogs[]=['ordernum'=>$orderMsg->ordernum,'user_id'=>$user->invited_id,'type'=>1,'credit'=>$credit,
                'get_type'=>2,'status'=>1,'remark'=>'下级订单返利：'.$credit,'created_at'=>$time,'updated_at'=>$time];
            $countPercent+=$upLevel['up'];
        }
        //team暂时返剩余全部 方便统计计算
        if($countPercent<100){
            $remain=100-$countPercent;
            $user->team_id=$user->team_id?:1;
            $credit=$orderMsg->calcuate_money*$remain*0.01;
            $userLogs[]=['ordernum'=>$orderMsg->ordernum,'user_id'=>$user->team_id,'type'=>1,'credit'=>$credit,'get_type'=>3,'status'=>1,'remark'=>'团队订单返利：'
                .$orderMsg->calcuate_money*$remain*0.01,'created_at'=>$time,'updated_at'=>$time];
        }
